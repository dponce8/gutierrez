<div style="height : <?php if (count($listado) < 12) {echo (60 + (count($listado) * 40));} else {echo '350';}?>px; overflow : auto; white-space: nowrap; overflow-x : auto;">
    <table class="table table-bordered" id="tabla_movimientos">
        ...
        <tr style="font-size: 10px;">
            ...
            <th style="font-size: 10px; font-weight: bold">Viaje N°</th>
            ...
            <th style="font-size: 10px; font-weight: bold">Pagado</th>
        ...
        <?php $c =1; foreach ($listado as $m) { ?>
            <tr class="table-light" id="fila_m_<?=$c?>" style="font-size: 12px; cursor: pointer; ">
                ...
                <td><?= $m['cliente']?></td>
                <td style="text-align: right; font-weight: bold; background-color: #d4edda; color: #155724;"><?= number_format(floatval($m['total']), 2, ',', '.')?></td>
                <td style="text-align: right; font-weight: bold; background-color: #cce7ff; color: #004085;"><?= number_format(floatval($m['importe_pagado']), 2, ',', '.')?></td>
                <td style="text-align: right; font-weight: bold; background-color: <?= (floatval($m['total']) - floatval($m['importe_pagado'])) > 0 ? '#fff3cd' : '#d1ecf1' ?>; color: <?= (floatval($m['total']) - floatval($m['importe_pagado'])) > 0 ? '#856404' : '#0c5460' ?>;"><?= number_format(floatval($m['total']) - floatval($m['importe_pagado']), 2, ',', '.')?></td>
